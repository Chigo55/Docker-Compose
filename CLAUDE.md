...
로드맵은 **GitHub Project "SQL Server Farm 로드맵"**(owner Chigo55, 저장소에 link됨)과 `roadmap` 라벨 이슈로 관리합니다. 신규 항목 등록·상태 갱신은 Project에서 합니다(완료 시 이슈 close → Status Done). 저장소 안에 로드맵 문서는 없습니다 — 동결 사본이던 `docs/ROADMAP.md`는 실제와 어긋나 삭제했고, 역사는 git 이 보존합니다([ADR-0019](.claude/adr/0019-remove-frozen-roadmap.md), 원문은 `git show v1.1.1:docs/ROADMAP.md`).

## 규칙 일부는 파일이 아니라 GitHub 설정에 있습니다

clone 해서는 보이지 않지만 작업을 막는 것들이 있습니다. 전체 지도는 [rules/github.md](.claude/rules/github.md)에 있고, 최소한 이 둘은 알고 시작하세요.
...
.github/
  ...
  pull_request_template.md  저장소 고유 함정 체크리스트
...
- Docker 가 꺼져 있으면 doctor 는 compose 렌더링만 건너뜁니다(성공 시 명시적 `exit 0` 이라 종료 코드가 새지 않습니다 — 이슈 #3 해결). 규약 위반 여부는 doctor 요약 `오류 N건` 으로 판단하세요.
...
5. **작업 후 정리** — 병합되면 원격 브랜치는 자동 삭제되고, 로컬은 `git worktree remove` 로 정리합니다.

> ⚠️ `ci.yml` 의 job `name:` 은 ruleset 의 필수 체크 이름과 **글자 그대로** 묶여 있습니다. 바꾸면 체크가 도착하지 않아 모든 PR 이 "대기 중"으로 멈춰 병합 불가가 됩니다 — 바꾸려면 ruleset 도 같은 PR 에서 함께 고치세요([rules/github.md](.claude/rules/github.md)).

추적은 문서가 아니라 GitHub 으로 합니다: **로드맵·추후 업데이트**는 위 [로드맵](#로드맵) 섹션대로 GitHub Project 로, **작업 중 발견한 범위 밖 버그·위험**은 `gh issue create` 로 등록합니다(현재 작업 커밋에 섞지 않음). Discussions 는 켜져 있으나 비어 있어(정리는 이슈 #26) 논의도 이슈로 합니다.
