...
데이터(backup/restore), 실행(query), 점검(doctor)을 갖췄습니다. 이 문서는 **다음에
무엇을 추가하면 운영이 편해지는가**를 우선순위와 함께 정리한 것입니다.
...
### 9. 테스트 · 린트 · CI 파이프라인
**문제**: 스크립트가 늘면서 회귀 위험이 커집니다. 지금은 자동 검증이 없습니다.

**가치**:
- **Pester**로 `_common.ps1` 순수 함수 단위 테스트(`Read-DotEnv` 파싱, `Get-Instances` 발견, `Resolve-Services` 오타 검증 등 — Docker 불필요한 부분).
- **PSScriptAnalyzer**로 스타일·안전 규칙 린트.
- **GitHub Actions**: PR마다 `doctor.ps1`(정적 부분) + Pester + PSScriptAnalyzer 실행.

```
.github/workflows/ci.yml   # windows-latest 러너에서 Pester + PSScriptAnalyzer
tests/_common.Tests.ps1    # Pester 테스트
```

- **주의**: Docker가 필요한 부분(백업/복원 실동작)은 CI에서 검증이 어렵습니다 → 순수 로직과 I/O를 분리해 테스트 가능 영역을 넓히는 리팩터가 선행되면 좋습니다.
- **노력도**: L
